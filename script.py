...
outbound_dates = ["2020-01-21", "2020-01-22", "2020-01-23", "2020-01-24", "2020-01-25", "2020-01-26", "2020-01-27"]
...
class CallPy(object):
	...
	def call_python_file(self, origin_city, destin_city, outbound_date, file):
		#">>", "./outputs/{}_{}_{}_{}.json".format(origin_city, destin_city, outbound_date, inbound_date)]
		call(["python3", "{}".format(self.path), "routes", origin_city, destin_city, outbound_date], stdout=file)

if __name__ == "__main__":
	c = CallPy()
	for trip in trips: 
		for i in range(len(outbound_dates)):
			print("Executing Trip from {} to {} leaving on {}".format(trip[0],trip[1], outbound_dates[i]))
			#try:
			f = open("./outputs_2/{}_{}_{}.json".format(trip[0],trip[1], outbound_dates[i]), "w")
			c.call_python_file(origin_city=trip[0], destin_city=trip[1], outbound_date=outbound_dates[i], file=f) 
			time.sleep(3)
			f.close()
